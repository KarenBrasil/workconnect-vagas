...
import {
  ...
  TouchableOpacity, FlatList, Linking, ActivityIndicator, Alert
} from 'react-native';
...
export default function Home() {
  ...
  const [termoBusca, setTermoBusca] = useState('desenvolvedor');
  ...
  const [filtro, setFiltro] = useState<'todos' | 'recrutador' | 'freelancer'>('todos');

  const userId = auth.currentUser?.uid || 'anonimo';

  useEffect(() => {
    carregarVagasInternas();
    carregarVagasExternas('desenvolvedor');
    carregarFavoritos();
  }, []);
  ...
  const handleBuscar = () => {
    if (busca.trim()) {
      ...
      carregarVagasExternas(busca.trim());
    }
  };
    ...
    <View style={styles.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.filtrosContainer}>
          {(['todos', 'recrutador', 'freelancer'] as const).map(f => (
            <TouchableOpacity
              key={f}
            ...
            >
              <Text style={[styles.filtroText, filtro === f && styles.filtroTextAtivo]}>
                {f === 'todos' ? 'Todos' : f === 'recrutador' ? '🏢 Vagas' : '💼 Freelancers'}
              </Text>
            </TouchableOpacity>
          ))}
        </View>

        {/* Vagas do WorkConnect */}
        <View style={styles.sectionHeader}>
          <Text style={styles.sectionTitle}>No WorkConnect</Text>
        ...
        {/* Vagas Externas */}
        <View style={styles.sectionHeader}>
          <Text style={styles.sectionTitle}>Vagas Externas</Text>
          <TouchableOpacity onPress={() => carregarVagasExternas(termoBusca)}>
            <Text style={styles.seeAll}>Atualizar</Text>
          </TouchableOpacity>
        </View>
        ...
        {loadingExternas ? (
          <ActivityIndicator color="#2E9D4D" style={{ marginVertical: 20 }} />
        ) : vagasExternas.length === 0 ? (
          <View style={styles.emptyCard}>
            <Text style={styles.emptyText}>Nenhuma vaga externa encontrada.</Text>
            <Text style={styles.emptySubText}>Tente buscar por outra palavra-chave.</Text>
          </View>
        ) : (
          vagasExternas.map(vaga => (
            <TouchableOpacity
              key={vaga.id}
              style={styles.card}
...
const styles = StyleSheet.create({
  ...
  filtrosContainer: { flexDirection: 'row', gap: 8, marginBottom: 24 },
  filtroBtn: {
  ...
  emptySubText: { fontSize: 14, color: '#83829A' },
});
